...
canal_Term_last_rope_analyzer::canal_Term_last_rope_analyzer(clang::ASTContext *c,function *f,scheduler *s): context(c), current_function(f), Schedule(s){
	debug("creating canal_Term_last_rope_analyzer");
}

bool canal_Term_last_rope_analyzer::VisitCompoundStmt(clang::CompoundStmt *comp){
	if(!Schedule.look_up())
		return true;
...
bool canal_Term_last_rope_analyzer::VisitArraySubscriptExpr(clang::ArraySubscriptExpr *e){
	if(!Schedule.look_up())
		return true;
	Schedule.increase_child();
	current_function->operations.insert_last_rope(Access(*current_function));
	current_function->operations.insert_last_rope(Add(*current_function));
	return true;
}

bool canal_Term_last_rope_analyzer::VisitIfStmt(clang::IfStmt *if_stmnt){
	if(!Schedule.look_up())
		return true;
	Schedule.increase_child();
...
bool canal_Term_last_rope_analyzer::VisitVarDecl(clang::VarDecl *var_decl){
	if(!Schedule.look_up())
		return true;
	Schedule.increase_child();
...
bool canal_Term_last_rope_analyzer::VisitReturnStmt(clang::ReturnStmt *ret){
	if(!Schedule.look_up())
		return true;
	Schedule.increase_child();
	...
	current_function->operations.insert_last_rope(Ret(*current_function));

	return true;
}

bool canal_Term_last_rope_analyzer::VisitUnaryOperator(clang::UnaryOperator *un_op){
	if(!Schedule.look_up())
		return true;
	Schedule.increase_child();
	...
	if(thisIsFollowupForAVarDecl){
		...
		current_function->operations.insert_last_rope(Equal(*current_function));
		current_function->operations.insert_last_rope(VarPush(var_name,*current_function));
	...
	switch(un_op->getOpcode()){
		...
		case clang::UnaryOperatorKind::UO_PostDec:
			current_function->operations.insert_last_rope(Dec_Post(*current_function));
			break;
		case clang::UnaryOperatorKind::UO_PreDec:
			current_function->operations.insert_last_rope(Dec_Pre(*current_function));
			break;
		case clang::UnaryOperatorKind::UO_PostInc:
			current_function->operations.insert_last_rope(Inc_Post(*current_function));
			break;
		case clang::UnaryOperatorKind::UO_PreInc:
			current_function->operations.insert_last_rope(Inc_Pre(*current_function));
			break;
		case clang::UnaryOperatorKind::UO_Not:
			current_function->operations.insert_last_rope(Neg(*current_function));
			break;
...
bool canal_Term_last_rope_analyzer::VisitCallExpr(clang::CallExpr *call_expr){
	if(!Schedule.look_up())
		return true;
	Schedule.increase_child();
	...
	current_function->operations.insert_last_rope(Call(NULL, *current_function));
	thisIsFollowupOfaCall = true;

	return true;
}

bool canal_Term_last_rope_analyzer::VisitDeclRefExpr(clang::DeclRefExpr *decl_ref){
	if(!Schedule.look_up())
		return true;
	Schedule.increase_child();
...
bool canal_Term_last_rope_analyzer::VisitIntegerLiteral(clang::IntegerLiteral *in_lt){
	if(!Schedule.look_up())
		return true;
	Schedule.increase_child();
	...
	current_function->operations.insert_last_rope(IntLiteral(literal,*current_function));

	return true;
}


bool canal_Term_last_rope_analyzer::VisitBinaryOperator(clang::BinaryOperator *bn_op){
	if(!Schedule.look_up())
		return true;
	Schedule.increase_child();
	...
	switch(bn_op->getOpcode()){
		...
		case clang::BinaryOperatorKind::BO_Sub:
			...
			current_function->operations.insert_last_rope(Minus(*current_function));
			break;
		case clang::BinaryOperatorKind::BO_Div:
			debug("found Divide operator");
			current_function->operations.insert_last_rope(Divide(*current_function));
			break;
		case clang::BinaryOperatorKind::BO_Mul:
			debug("found Times operator");
			current_function->operations.insert_last_rope(Times(*current_function));
			break;
		case clang::BinaryOperatorKind::BO_Or:
			debug("found Or operator");
			current_function->operations.insert_last_rope(Or(*current_function));
			break;
		case clang::BinaryOperatorKind::BO_And:
			debug("found And operator");
			current_function->operations.insert_last_rope(And(*current_function));
			break;
		case clang::BinaryOperatorKind::BO_Xor:
			debug("found Xor operator");
			current_function->operations.insert_last_rope(Xor(*current_function));
			break;
		case clang::BinaryOperatorKind::BO_Assign:
			...
			if(isAccessInStmt(bn_op->getLHS(),context)){
				...
				if(isAccessInStmt(bn_op->getRHS(), context)){
					canal_Term_last_rope_analyzer analyzer(context,current_function);
		...
		case clang::BinaryOperatorKind::BO_Shl:
			...
			current_function->operations.insert_last_rope(Shl(*current_function));
			break;
		case clang::BinaryOperatorKind::BO_Shr:
			debug("found Shr operator");
			current_function->operations.insert_last_rope(Shr(*current_function));
			break;
		default:
...
void canal_Term_last_rope_analyzer::addVar(const std::string &name, std::string &type){
	if(type == "double"){
